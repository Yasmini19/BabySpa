                        ...
                        <tbody> 
                            ...
                            <?php foreach ($pemesanandetail as $row): ?>
                                ...
                                <tr>
                                    ...
                                    <input class="jumlahsisa" type="hidden" name="jumlahsisa[]" value="<?php echo $row['jumlahsisa'] ?>">
                                    <td><?php echo $row['item'] ?></td>
                                    <td class="text-right" width="12%">
                                        <input type="text" name="harga[]" maxlength="9" class="form-control harga text-right decimalnumber" value="<?php echo number_format($row['harga']) ?>">
                                    </td>
                                    <td class="text-right" width="12%">
                                        <input type="number" min="0" name="jumlah[]" class="form-control jumlah text-right" value="<?php echo number_format($row['jumlahsisa']) ?>" required>
                                    </td>
                                    <td class="text-right" width="12%">
                                        <input type="text" class="form-control subtotal text-right" value="<?php echo number_format($subtotal) ?>" disabled>
                                    </td>
                                    <td class="text-right" width="12%">
                                        <input type="text" name="diskon[]" class="form-control diskon text-right" value="<?php echo number_format($row['diskon']) ?>">
                                    </td>
                                    <td class="text-right" width="12%">
                                        <input type="text" name="ppn[]" class="form-control ppn text-right" value="<?php echo number_format($row['ppn']) ?>">
                                    </td>
                                    <td class="text-right" width="12%">
...
<script type="text/javascript">
    ...
    $('#table_detail tbody').on('change','.harga, .jumlah, .diskon, .ppn',function(){
        ...
        if(jumlah < 0 || harga <= 0 || ppn < 0 || diskon < 0) {
            NotifyError('Tidak boleh minus!');
            location.reload()
        }
    ...
    function save() {
        ...
        var harga = 0;
        $('#table_detail tbody .harga').each(function() {
        ...
        if(harga <= 0) {
            NotifyError('Silahkan isi harga yang benar!');
            return false;
        }
        var jumlah = 0;
        ...
        if(jumlah <= 0) {
            NotifyError('Silahkan isi jumlah yang benar!');
            return false;
        }
